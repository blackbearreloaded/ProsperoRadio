...
ProsperoRadio decodes MP3 with the console-provided `libSceAudiodec` codec type
...
[`mp3_header.hpp`](../include/mp3_header.hpp) therefore validates MPEG-1, MPEG-2,
...
| 2026-08-26 | 6.02 | `PPSA99651` | The same probe wrote `PROSPERO_RADIO_MP3 stage=playing result=0 rate=48000 channels=2 bytes=4608` into title-local storage. The runner observed the title for 60 seconds, closed it normally, and confirmed runtime release. |
...
`tools/mp3_header_check.cpp` covers MPEG-1, MPEG-2, and MPEG-2.5 headers,
44.1/24/11.025 kHz rates, mono and stereo, padded frames, prefixed metadata,
and reserved version/layer rejection. `tools/icy_metadata_check.cpp` covers
...
`tools/radio_service_json_check.cpp` confirms that a healthy non-HLS Radio Browser
