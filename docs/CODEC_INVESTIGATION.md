...
This document records ProsperoRadio's hardware-first codec investigation. It
...
This document keeps the ProsperoRadio-specific decisions and integration boundaries.
...
| MP3 | Public codec `0x0002` reaches AJM and is hardware-validated in ProsperoRadio. | Keep the native decoder. |
| Ogg Opus | `libSceOpusDec` and `libSceOpusCeltDec` reach AJMI/AJM and are hardware-validated in ProsperoRadio. | Keep the native dual-decoder path. |
...
This is a measured platform boundary, not a container-parser failure. ProsperoRadio
...
```text
PROSPERO_RADIO_FLAC_NATIVE module=0x80000053 load=-2141581312 load_hex=0x805a1000 unload=0
```
...
directly to ProsperoRadio's cancellable network producer without adding libogg or a
new I/O abstraction.
...
individual frame. ProsperoRadio therefore enforces its own memory ceiling and fails
...
ProsperoRadio needs only a small adaptation that accepts an audio-only PMT with one
...
demuxed output for the captured segment. For `mp4a.40.29` manifests, ProsperoRadio
