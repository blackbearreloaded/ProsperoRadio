...
ProsperoRadio uses a bounded CPU fallback for Ogg Vorbis because the hardware-first
...
The decoder is isolated behind `src/vorbis_decoder.cpp` and uses the upstream
...
`tools/vorbis_decoder_check.cpp` decodes a deterministic Ogg Vorbis fixture with
