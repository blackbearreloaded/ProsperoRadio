...
The ProsperoRadio layer supplies the RmlUi/SDL2 UI, Radio Browser service, SQLite
...
The root [Makefile](../Makefile) declares the ProsperoRadio-specific build inputs:
...
`tooling/native/prospero_radio_import_stub_*.cpp`; they declare only the C ABI
symbols ProsperoRadio actually imports. They contain no decoder implementation,
...
single `{{PROSPERO_RADIO_VERSION}}` placeholder in the copied RML document with
