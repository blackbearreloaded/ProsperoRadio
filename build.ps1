...
$uiPath = Join-Path $ProjectDirectory "ui"
...
foreach ($source in @($project.sources)) {
    ...
    $compiler = if ([IO.Path]::GetExtension($source) -eq ".c") {
        "sh tooling/prospero-clang18"
    } else {
        "bash /opt/ps5-payload-sdk/bin/prospero-clang++"
    }
    $definitionFlags = ($compileDefinitions | ForEach-Object { "-D$_" }) -join " "
    $includeFlags = ($includePaths | ForEach-Object { "-I$_" }) -join " "
    $compile = "cd '$wslRoot' && LLVM_CONFIG=/usr/bin/llvm-config-18 $compiler $languageFlags -O2 -g -Wall -Wextra -Werror -ffunction-sections -fdata-sections $definitionFlags $includeFlags -c '$sourceFromRepo' -o '$wslObject'"
...
foreach ($stub in $stubLibraries) {
    $linkArguments += @("--stub", [IO.Path]::GetFullPath((Join-Path $here $stub)))
}
$linkArguments += @(
    "--out", $rawModule,
    "--entry", "_start",
