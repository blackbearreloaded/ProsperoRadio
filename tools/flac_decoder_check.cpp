...
{
    auto *reader = static_cast<memory_reader_t *>(context);
...
{
    auto *reader = static_cast<oversized_metadata_reader_t *>(context);
...
int main(void)
{
    ...
    auto *oversized_block = static_cast<uint8_t *>(malloc(FLAC_FIXTURE_len));
    check(oversized_block != nullptr, "oversized-block fixture allocation");
