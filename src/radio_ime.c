...
static void utf8_to_utf16(const char * source, uint16_t * output, size_t capacity)
{
    ...
    while(*text != 0 && written + 1U < capacity) {
        ...
        unsigned bytes = 1;
        if((text[0] & 0xe0U) == 0xc0U) { codepoint = text[0] & 0x1fU; bytes = 2; }
        else if((text[0] & 0xf0U) == 0xe0U) { codepoint = text[0] & 0x0fU; bytes = 3; }
        else if((text[0] & 0xf8U) == 0xf0U) { codepoint = text[0] & 0x07U; bytes = 4; }
        bool valid = bytes == 1;
        if(bytes > 1) {
            valid = true;
            for(unsigned i = 1; i < bytes; ++i) {
                if(text[i] == 0 || (text[i] & 0xc0U) != 0x80U) { valid = false; break; }
                codepoint = (codepoint << 6) | (text[i] & 0x3fU);
            }
            const uint32_t minimum = bytes == 2 ? 0x80U : bytes == 3 ? 0x800U : 0x10000U;
            if(codepoint < minimum || codepoint > 0x10ffffU ||
               (codepoint >= 0xd800U && codepoint <= 0xdfffU)) valid = false;
        }
        if(!valid) { codepoint = 0xfffdU; bytes = 1; }
        text += bytes;
        if(codepoint >= 0x10000U) {
            if(written + 2U >= capacity) break;
            codepoint -= 0x10000U;
            output[written++] = (uint16_t)(0xd800U | (codepoint >> 10));
            output[written++] = (uint16_t)(0xdc00U | (codepoint & 0x3ffU));
        }
        else output[written++] = (uint16_t)codepoint;
    }
    output[written] = 0;
}
...
static void utf16_to_utf8(const uint16_t * source, char * output, size_t capacity)
{
    ...
    for(size_t i = 0; source[i] != 0; ++i) {
        ...
        if(codepoint >= 0xd800U && codepoint <= 0xdbffU &&
           source[i + 1U] >= 0xdc00U && source[i + 1U] <= 0xdfffU) {
            codepoint = 0x10000U + ((codepoint - 0xd800U) << 10) +
                        (source[++i] - 0xdc00U);
        }
        else if(codepoint >= 0xd800U && codepoint <= 0xdfffU) codepoint = 0xfffdU;
        const unsigned bytes = codepoint < 0x80U ? 1U : codepoint < 0x800U ? 2U :
                               codepoint < 0x10000U ? 3U : 4U;
        if(written + bytes >= capacity) break;
        if(bytes == 1U) output[written++] = (char)codepoint;
        else {
            for(unsigned byte = bytes - 1U; byte != 0; --byte) {
                output[written + byte] = (char)(0x80U |
                    ((codepoint >> (6U * (bytes - 1U - byte))) & 0x3fU));
            }
            output[written] = (char)((0xf0U << (4U - bytes)) |
                                      (codepoint >> (6U * (bytes - 1U))));
            written += bytes;
        }
    }
    output[written] = '\0';
...
bool radio_ime_init(void)
{
    ...
    if(common_dialog < 0 && (uint32_t)common_dialog != SCE_COMMON_DIALOG_ALREADY_INITIALIZED)
        return false;
    if(sceSysmoduleLoadModule(SCE_SYSMODULE_IME_DIALOG) < 0) return false;
    module_loaded = true;
    return true;
}
...
{
    if(active || requested || !module_loaded) return;
...
{
    int32_t user_id = -1;
    if(sceUserServiceGetForegroundUser(&user_id) < 0) { requested = false; return; }
    ...
    const sce_ime_dialog_param_t param = {
        ...
        .type = 0,
        .enter_label = 2,
        .max_text_length = IME_TEXT_CHARACTERS,
...
{
    if(requested && !radio_input_pressed(RADIO_INPUT_CROSS)) start_requested();
    if(!active) return;

    const int status = sceImeDialogGetStatus();
    if(status == 1 || (status == 0 && SDL_GetTicks() - started_at < 1000U)) return;
    if(status == 2) {
