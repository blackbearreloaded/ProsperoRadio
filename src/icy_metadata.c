...
static bool equal_case(const char * value, size_t size, const char * text)
{
    ...
    for(size_t i = 0U; i < size; ++i) {
        if(ascii_lower(value[i]) != ascii_lower(text[i])) return false;
    }
    return true;
}
...
{
    if(headers == NULL) return 0U;
    size_t position = 0U;
    while(position < size) {
        const size_t start = position;
        while(position < size && headers[position] != '\r' &&
              headers[position] != '\n') ++position;
        ...
        if(colon == end || !equal_case(headers + start, colon - start,
                                       "icy-metaint")) continue;

        size_t at = colon + 1U;
        while(at < end && (headers[at] == ' ' || headers[at] == '\t')) ++at;
        size_t interval = 0U;
        const size_t digits = at;
        while(at < end && headers[at] >= '0' && headers[at] <= '9') {
            const size_t digit = (size_t)(headers[at] - '0');
            if(interval > (SIZE_MAX - digit) / 10U) return 0U;
...
{
    icy_metadata_reader_t * reader = context;
    if(reader == NULL || reader->read == NULL || (data == NULL && size != 0U))
        return -1;
    if(size == 0U) return 0;
    ...
    for(;;) {
        if(reader->metadata_remaining != 0U) {
            const size_t wanted = reader->metadata_remaining < sizeof(discard)
                ? reader->metadata_remaining : sizeof(discard);
            const int received = reader->read(reader->context, discard, wanted);
            if(received <= 0) return received;
            if((size_t)received > wanted) return -1;
        ...
        const size_t wanted = size < reader->audio_remaining
            ? size : reader->audio_remaining;
