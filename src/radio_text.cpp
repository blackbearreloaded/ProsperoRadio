...
Direction Classify(std::uint32_t codepoint) {
    if (IsRtl(codepoint)) return Direction::Right;
    if (codepoint <= 0x20 || (codepoint >= 0x2000 && codepoint <= 0x206f) ||
        (codepoint < 0x80 && !((codepoint >= '0' && codepoint <= '9') ||
...
std::vector<std::uint32_t> Decode(const char* text) {
    ...
    while (*input) {
        ...
        if ((value & 0xe0) == 0xc0) { value &= 0x1f; continuation = 1; }
        else if ((value & 0xf0) == 0xe0) { value &= 0x0f; continuation = 2; }
        else if ((value & 0xf8) == 0xf0) { value &= 0x07; continuation = 3; }
        else if (value >= 0x80) { result.push_back(0xfffd); continue; }
        bool valid = true;
        for (unsigned index = 0; index < continuation; ++index) {
            if ((input[index] & 0xc0) != 0x80) { valid = false; break; }
            value = (value << 6) | (input[index] & 0x3f);
        }
        if (!valid) result.push_back(0xfffd);
        else { input += continuation; result.push_back(value); }
    }
    return result;
}
...
int PreviousLetter(const std::vector<std::uint32_t>& text, int at) {
    for (--at; at >= 0; --at)
        if (!IsArabicMark(text[static_cast<std::size_t>(at)])) return at;
    return -1;
}
...
std::uint32_t LamAlef(std::uint32_t codepoint, bool connected) {
    switch (codepoint) {
        case 0x0622: return connected ? 0xfef6 : 0xfef5;
        case 0x0623: return connected ? 0xfef8 : 0xfef7;
        case 0x0625: return connected ? 0xfefa : 0xfef9;
        case 0x0627: return connected ? 0xfefc : 0xfefb;
        default: return 0;
    }
}
...
void ShapeArabic(std::vector<std::uint32_t>& text) {
    ...
    for (int at = 0; at < static_cast<int>(source.size()); ++at) {
        ...
        const ArabicForm* previous = previous_at >= 0
            ? FindArabic(source[static_cast<std::size_t>(previous_at)]) : nullptr;
        const ArabicForm* next = next_at >= 0
            ? FindArabic(source[static_cast<std::size_t>(next_at)]) : nullptr;
        const bool joins_previous = current->final && previous && previous->initial;

        if (current->base == 0x0644 && next_at == at + 1) {
            const std::uint32_t ligature = LamAlef(
                source[static_cast<std::size_t>(next_at)], joins_previous);
            if (ligature) {
        ...
        if (joins_previous && joins_next && current->medial) shaped.push_back(current->medial);
        else if (joins_previous) shaped.push_back(current->final);
        else if (joins_next) shaped.push_back(current->initial);
        else shaped.push_back(current->isolated);
    }
...
std::string ShapeLine(const char* text) {
    ...
    for (std::uint32_t glyph : glyphs) {
        ...
        if (base == Direction::Neutral && direction != Direction::Neutral) base = direction;
        if (direction == Direction::Right) has_rtl = true;
    }
    if (!has_rtl) return text ? text : "";
    ...
    for (std::size_t index = 0; index < glyphs.size(); ++index) {
        ...
        if (directions[index] == Direction::Neutral) directions[index] = previous;
        else previous = directions[index];
    }
    Direction next = base;
    for (std::size_t index = directions.size(); index-- > 0;) {
        if (directions[index] == Direction::Neutral) directions[index] = next;
        else next = directions[index];
    }

    struct Run { std::size_t begin; std::size_t end; Direction direction; };
    std::vector<Run> runs;
    for (std::size_t begin = 0; begin < glyphs.size();) {
        std::size_t end = begin + 1;
        while (end < glyphs.size() && directions[end] == directions[begin]) ++end;
    ...
    auto append_run = [&](const Run& run) {
        if (run.direction == Direction::Right) {
            for (std::size_t index = run.end; index-- > run.begin;) Encode(output, glyphs[index]);
        } else {
            for (std::size_t index = run.begin; index < run.end; ++index) Encode(output, glyphs[index]);
        }
    };
    if (base == Direction::Right) {
        for (std::size_t index = runs.size(); index-- > 0;) append_run(runs[index]);
    } else {
        for (const Run& run : runs) append_run(run);
    }
    return output;
}
