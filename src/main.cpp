...
bool LoadFonts() {
    return Rml::LoadFontFace("ui/fonts/Inter-Regular.ttf") &&
        Rml::LoadFontFace("ui/fonts/Inter-SemiBold.ttf") &&
...
bool RunSmoke() {
    ...
    SDL_Window* window = SDL_CreateWindow("Radio Browser PPSA99746", SDL_WINDOWPOS_UNDEFINED,
    ...
    while (running) {
        ...
        SDL_UpdateWindowSurface(window);
        if (!screenshot_saved) {
            screenshot_saved = SDL_SaveBMP(surface, "/download0/PPSA99746-ui.bmp") == 0;
        }
