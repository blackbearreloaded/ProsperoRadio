...
extern int sceOpusCeltDecCreateEx(uint32_t * context, void * state,
                                  int sample_rate, int channels);
extern int sceOpusCeltDecDecode(void * state, const uint8_t * packet,
                                int packet_bytes, int16_t * pcm,
                                int pcm_capacity_bytes);
                      ...
                      bool celt_only)
{
    ...
    const uint32_t module = celt_only ? OPUS_CELT_DECODER_MODULE
                                      : OPUS_DECODER_MODULE;
    int result = sceSysmoduleLoadModuleInternal(module);
    if(result < 0) return result;
    decoder->module_loaded = true;

    const int state_size = celt_only ? sceOpusCeltDecGetSize((int)channels)
                                     : sceOpusDecGetSize((int)channels);
    ...
    result = celt_only
        ? sceOpusCeltDecCreateEx(&decoder->context, decoder->state,
                                 48000, (int)channels)
        : sceOpusDecCreateEx(&decoder->context, decoder->state,
                             48000, (int)channels);
    if(result < 0) {
        opus_decoder_close(decoder);
        return result;
                        ...
                        size_t * produced_bytes)
{
    ...
    const int result = decoder->celt_only
        ? sceOpusCeltDecDecode(decoder->state, packet, (int)packet_size,
                               pcm, (int)pcm_capacity_bytes)
        : sceOpusDecDecode(decoder->state, packet, (int)packet_size,
                           pcm, (int)pcm_capacity_bytes);
    if(result < 0) return result;
...
void opus_decoder_close(opus_decoder_t * decoder)
{
    ...
    if(decoder->created) {
        if(decoder->celt_only) sceOpusCeltDecDestroy(decoder->state);
        else sceOpusDecDestroy(decoder->state);
    }
    if(decoder->initialized) {
        if(decoder->celt_only) sceOpusCeltDecTerminate(&decoder->context);
        else sceOpusDecTerminate(&decoder->context);
    }
    free(decoder->allocation);
    if(decoder->module_loaded) {
        sceSysmoduleUnloadModuleInternal(
            decoder->celt_only ? OPUS_CELT_DECODER_MODULE : OPUS_DECODER_MODULE);
    }
    memset(decoder, 0, sizeof(*decoder));
}
