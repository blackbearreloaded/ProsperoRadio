...
{
    switch(result) {
        case OGG_PAGE_OK: return OGG_STREAM_OK;
        case OGG_PAGE_ERR_ARGUMENT: return OGG_STREAM_ERR_ARGUMENT;
        case OGG_PAGE_ERR_CAPTURE: return OGG_STREAM_ERR_CAPTURE;
        case OGG_PAGE_ERR_VERSION: return OGG_STREAM_ERR_VERSION;
        case OGG_PAGE_ERR_CHECKSUM: return OGG_STREAM_ERR_CHECKSUM;
        case OGG_PAGE_ERR_TRUNCATED: return OGG_STREAM_ERR_TRUNCATED;
        default: return OGG_STREAM_ERR_PAGE;
    }
}
...
static int pull_page(ogg_stream_t * stream)
{
    ...
    while(stream->page_size == 0U) {
        ...
        if(needed > sizeof(input)) needed = sizeof(input);
        const int count = stream->source_read(stream->source_context, input,
                                              needed);
        ...
        if(count == 0) {
            ...
            stream->status = result == OGG_PAGE_OK
                ? OGG_STREAM_EOF : page_error(result);
            return stream->status == OGG_STREAM_EOF ? 0 : -1;
        }

        const ogg_page_result_t result =
            ogg_page_feed(&stream->parser, input, (size_t)count);
        if(result != OGG_PAGE_OK) {
            if(result != OGG_PAGE_ERR_CALLBACK ||
                     ...
                     void * source_context)
{
    ...
    stream->status = source_read != NULL
        ? OGG_STREAM_OK : OGG_STREAM_ERR_ARGUMENT;
...
int ogg_stream_read(void * context, void * output, size_t size)
{
    ...
    if(stream->status < OGG_STREAM_OK) return -1;
    if(stream->status != OGG_STREAM_OK || size == 0U) return 0;
