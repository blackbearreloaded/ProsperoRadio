...
static bool attribute(hls_slice_t list, const char *wanted, hls_slice_t *output)
{
    size_t position = 0;
    while (position < list.size)
    {
        ...
        if (position >= list.size || list.data[position] != '=')
            return false;
        hls_slice_t key = trim((hls_slice_t){list.data + key_start, position - key_start});
        ++position;
        hls_slice_t value;
        if (position < list.size && list.data[position] == '"')
        {
...
static bool audio_only_codecs(hls_slice_t attributes, uint32_t *source_channels)
{
    hls_slice_t codecs;
    *source_channels = 0U;
    if (!attribute(attributes, "CODECS", &codecs))
        return true;
    size_t position = 0;
    ...
    {
        const size_t start = position;
        while (position < codecs.size && codecs.data[position] != ',')
            ++position;
        hls_slice_t codec = trim((hls_slice_t){codecs.data + start, position - start});
        if (!starts_case(codec, "mp4a"))
            return false;
        if (equal_case(codec, "mp4a.40.29"))
            *source_channels = 2U;
        found_aac = true;
        if (position < codecs.size)
            ++position;
    }
    return found_aac;
}
...
static bool fmp4_url(hls_slice_t value)
{
    ...
    for (size_t i = 0; i < sizeof(extensions) / sizeof(extensions[0]); ++i)
    {
        ...
        if (end >= length &&
            starts_case((hls_slice_t){value.data + end - length, length}, extensions[i]))
            return true;
    }
    return false;
}

static radio_hls_result_t resolve(hls_slice_t reference, const char *playlist_url, char *output,
                                  size_t output_size)
{
    char text[RADIO_HLS_URL_BYTES];
    if (reference.size + 1U > sizeof(text))
        return RADIO_HLS_LIMIT;
    memcpy(text, reference.data, reference.size);
    text[reference.size] = '\0';
    const radio_playlist_result_t result =
        radio_playlist_resolve_url(playlist_url, text, output, output_size);
    return result == RADIO_PLAYLIST_OK      ? RADIO_HLS_OK
           : result == RADIO_PLAYLIST_LIMIT ? RADIO_HLS_LIMIT
                                            : RADIO_HLS_INVALID;
}

radio_hls_result_t radio_hls_parse(const char *data, size_t size, const char *playlist_url,
                                   radio_hls_playlist_t *playlist)
{
    if (data == nullptr || size == 0 || playlist_url == nullptr || playlist == nullptr)
        return RADIO_HLS_INVALID;
    if (size > HLS_MAX_INPUT || memchr(data, '\0', size) != nullptr)
        return RADIO_HLS_LIMIT;
    ...
    while (position < size)
    {
        ...
        while (end < size && data[end] != '\n')
            ++end;
        if (end - position > HLS_MAX_LINE)
            return RADIO_HLS_LIMIT;
        hls_slice_t line = trim((hls_slice_t){data + position, end - position});
        if (!header && line.size >= 3U && memcmp(line.data, "\xef\xbb\xbf", 3U) == 0)
        {
            line.data += 3U;
            line.size -= 3U;
            line = trim(line);
        ...
        if (tag_value(line, "#EXT-X-STREAM-INF:", &value))
        {
            if (playlist->kind == RADIO_HLS_MEDIA || pending_variant ||
                !attribute(value, "BANDWIDTH", &line) || !parse_u64(line, &variant_bandwidth) ||
                variant_bandwidth == 0 || !audio_only_codecs(value, &variant_source_channels))
        ...
        {
            if (playlist->kind == RADIO_HLS_MASTER || !parse_u64(value, &sequence))
                return RADIO_HLS_MALFORMED;
        ...
        {
            uint64_t seconds = 0;
            if (playlist->kind == RADIO_HLS_MASTER || !parse_u64(value, &seconds) || seconds == 0 ||
        ...
        else if (starts(line, "#EXT-X-BYTERANGE:") || starts(line, "#EXT-X-MAP:") ||
                 starts(line, "#EXT-X-PART:") || starts(line, "#EXT-X-PRELOAD-HINT:") ||
                 starts(line, "#EXT-X-SKIP:") || starts(line, "#EXT-X-MEDIA:"))
        {
            return RADIO_HLS_UNSUPPORTED;
        }
        ...
        else if (line.data[0] != '#')
        {
            if (pending_variant)
            {
                if (playlist->variant_count >= RADIO_HLS_MAX_VARIANTS)
                    return RADIO_HLS_LIMIT;
                radio_hls_variant_t *variant = &playlist->variants[playlist->variant_count++];
                variant->bandwidth = variant_bandwidth;
                variant->source_channels = variant_source_channels;
                const radio_hls_result_t result =
                    resolve(line, playlist_url, variant->url, sizeof(variant->url));
                if (result != RADIO_HLS_OK)
                    return result;
...
{
    if (playlist == nullptr || playlist->kind != RADIO_HLS_MASTER || playlist->variant_count == 0)
        return RADIO_HLS_NO_VARIANT;
    uint32_t selected = 0;
    for (uint32_t i = 1; i < playlist->variant_count; ++i)
    {
        if (playlist->variants[i].bandwidth < playlist->variants[selected].bandwidth)
            selected = i;
    }
    return (int)selected;
}
