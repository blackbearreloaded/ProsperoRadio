...
void SetMultilineText(Rml::ElementDocument* document, const char* id, const char* text) {
    ...
    if (Rml::Element* element = Find(document, id)) element->SetInnerRML(encoded);
}

void SetClass(Rml::ElementDocument* document, const char* id,
    const char* class_name, bool enabled) {
    if (Rml::Element* element = Find(document, id)) element->SetClass(class_name, enabled);
}
...
void SetProperty(Rml::ElementDocument* document, const char* id,
    const char* name, const char* value) {
    if (Rml::Element* element = Find(document, id)) element->SetProperty(name, value);
}

void SetPixelProperty(Rml::ElementDocument* document, const char* id,
...
void FirstValue(const char* values, char* output, std::size_t capacity) {
    ...
    while (count && output[count - 1] == ' ') --count;
    output[count] = '\0';
}
...
bool PlaybackActive(radio_playback_state_t state) {
    return state == RADIO_PLAYBACK_CONNECTING ||
        state == RADIO_PLAYBACK_BUFFERING ||
        state == RADIO_PLAYBACK_PLAYING ||
        state == RADIO_PLAYBACK_STOPPING;
}

const char* StationColor(const char* uuid) {
    static const char* palette[] = {
        "#2c6d87", "#b25535", "#3d7e68", "#6a4b91",
        "#8a6539", "#35628a", "#864d65", "#497556"
    };
    unsigned hash = 0;
    for (; *uuid; ++uuid) hash = hash * 33U + static_cast<unsigned char>(*uuid);
...
bool RadioApp::Initialize(Rml::ElementDocument* document) {
    ...
    for (unsigned& station : card_stations_) station = InvalidStation;
    radio_service_init();
...
void RadioApp::RebuildFacets() {
    std::memset(country_facets_, 0, sizeof(country_facets_));
    std::memset(genre_facets_, 0, sizeof(genre_facets_));
    std::memset(language_facets_, 0, sizeof(language_facets_));
    country_facet_count_ = genre_facet_count_ = language_facet_count_ = 0;

    auto add = [](Facet* facets, unsigned& count, const char* value) {
        if (!*value) return;
        for (unsigned i = 0; i < count; ++i) {
            if (std::strcmp(facets[i].value, value) == 0) {
                ++facets[i].count;
                return;
            }
        }
        if (count < FacetMax) {
            CopyString(facets[count].value, sizeof(facets[count].value), value);
            facets[count].count = 1;
            ++count;
        }
    };
    auto sort = [](Facet* facets, unsigned count) {
        for (unsigned i = 1; i < count; ++i) {
            const Facet value = facets[i];
            unsigned at = i;
            while (at && facets[at - 1].count < value.count) {
                facets[at] = facets[at - 1];
                --at;
            }
            facets[at] = value;
        }
    ...
    for (unsigned i = 0; i < status.station_count; ++i) {
        ...
        if (!radio_service_get_station(i, &station)) continue;
        add(country_facets_, country_facet_count_, station.country_code);
        char value[48];
        FirstValue(station.tags, value, sizeof(value));
        add(genre_facets_, genre_facet_count_, value);
        FirstValue(station.language, value, sizeof(value));
        add(language_facets_, language_facet_count_, value);
    }
    sort(country_facets_, country_facet_count_);
    sort(genre_facets_, genre_facet_count_);
    sort(language_facets_, language_facet_count_);

    auto retained = [](const Facet* facets, unsigned count, const char* selected) {
        if (!*selected) return true;
        for (unsigned i = 0; i < count; ++i)
            if (std::strcmp(facets[i].value, selected) == 0) return true;
        return false;
    };
    if (!retained(country_facets_, country_facet_count_, filter_country_))
        filter_country_[0] = '\0';
    if (!retained(genre_facets_, genre_facet_count_, filter_genre_))
        filter_genre_[0] = '\0';
...
bool RadioApp::StationVisible(const radio_station_t& station) const {
    if (view_ == View::Popular) return station.popular_rank != RADIO_RANK_NONE;
    if (view_ == View::Trending) return station.trending_rank != RADIO_RANK_NONE;
    if (view_ == View::Voted) return station.voted_rank != RADIO_RANK_NONE;
    if (view_ == View::Favorites) return radio_service_is_favorite(station.uuid);

    if (*filter_country_ && std::strcmp(station.country_code, filter_country_) != 0)
        return false;
    if (*filter_genre_ && !ContainsCi(station.tags, filter_genre_)) return false;
    if (*filter_language_ && !ContainsCi(station.language, filter_language_)) return false;
    if (filter_bitrate_ && station.bitrate < filter_bitrate_) return false;
    if (!*search_query_) return true;
    return ContainsCi(station.name, search_query_) ||
        ContainsCi(station.tags, search_query_) ||
        ContainsCi(station.country, search_query_) ||
        ContainsCi(station.state, search_query_) ||
...
void RadioApp::BuildVisibleList() {
    ...
    radio_service_get_status(&status);
    visible_count_ = 0;
    for (unsigned i = 0; i < status.station_count && visible_count_ < RADIO_MAX_STATIONS; ++i) {
        radio_station_t station{};
        if (radio_service_get_station(i, &station) && StationVisible(station))
            visible_indices_[visible_count_++] = i;
    }

    if (view_ == View::Popular || view_ == View::Trending || view_ == View::Voted) {
        auto rank = [this](const radio_station_t& station) {
            if (view_ == View::Popular) return station.popular_rank;
            if (view_ == View::Trending) return station.trending_rank;
            return station.voted_rank;
        };
        for (unsigned i = 1; i < visible_count_; ++i) {
            const unsigned value = visible_indices_[i];
            radio_station_t value_station{};
            radio_service_get_station(value, &value_station);
            const unsigned value_rank = rank(value_station);
            unsigned at = i;
            while (at) {
                radio_station_t previous{};
                radio_service_get_station(visible_indices_[at - 1], &previous);
                if (rank(previous) <= value_rank) break;
                visible_indices_[at] = visible_indices_[at - 1];
                --at;
            }
            visible_indices_[at] = value;
        }
    }
    if (page_start_ >= visible_count_) page_start_ = 0;
    if (selected_slot_ >= CardCount) selected_slot_ = 0;
}
...
void RadioApp::RefreshHeading() {
    static const char* titles[] = {
        "Popular stations", "Trending now", "Top rated", "Your favorites", "Discover"
    };
    ...
    const unsigned page_count = visible_count_ ?
        (visible_count_ + CardCount - 1) / CardCount : 0;
    const unsigned page_number = visible_count_ ? page_start_ / CardCount + 1 : 0;
    char text[112];
    if (!visible_count_) CopyString(text, sizeof(text), "No matching stations");
    else {
        const unsigned last = page_start_ + CardCount < visible_count_ ?
            page_start_ + CardCount : visible_count_;
        std::snprintf(text, sizeof(text), "Page %u of %u   /   %u-%u of %u stations",
...
void RadioApp::RefreshDiscover() {
    ...
    const char* values[] = {
        *filter_country_ ? filter_country_ : "All countries",
        *filter_genre_ ? filter_genre_ : "All genres",
        *filter_language_ ? filter_language_ : "All languages"
    };
    const char* names[] = {"COUNTRY", "GENRE", "LANGUAGE"};
    const unsigned choices[] = {
        country_facet_count_, genre_facet_count_, language_facet_count_
    };
    auto count_for = [](const Facet* facets, unsigned count, const char* selected,
        unsigned all_count) {
        if (!*selected) return all_count;
        for (unsigned i = 0; i < count; ++i)
            if (std::strcmp(facets[i].value, selected) == 0) return facets[i].count;
        return 0U;
    };
    const unsigned matches[] = {
        count_for(country_facets_, country_facet_count_, filter_country_, choices[0]),
        count_for(genre_facets_, genre_facet_count_, filter_genre_, choices[1]),
        count_for(language_facets_, language_facet_count_, filter_language_, choices[2])
    };
    for (unsigned i = 0; i < 3; ++i) {
        const bool selected = (i == 0 && *filter_country_) ||
            (i == 1 && *filter_genre_) || (i == 2 && *filter_language_);
        char text[120];
...
void RadioApp::RefreshCard(unsigned slot) {
    ...
    if (page_start_ + slot >= visible_count_) {
        card_stations_[slot] = InvalidStation;
        SetVisible(document_, id, false);
        return;
    }

    const unsigned index = visible_indices_[page_start_ + slot];
    radio_station_t station{};
    ...
    if (!*tag) CopyString(tag, sizeof(tag), "Music");
    char text[192];
    std::snprintf(text, sizeof(text), "%s  |  %s  |  %s %u kbps",
        *station.country_code ? station.country_code : "World",
        tag, station.codec, station.bitrate);
    std::snprintf(id, sizeof(id), "meta-%u", slot);
    SetText(document_, id, text);

    if (view_ == View::Popular)
        std::snprintf(text, sizeof(text), "#%u  %u plays",
            station.popular_rank + 1, station.click_count);
    else if (view_ == View::Trending)
        std::snprintf(text, sizeof(text), "#%u  %+d",
            station.trending_rank + 1, station.click_trend);
    else if (view_ == View::Voted)
        std::snprintf(text, sizeof(text), "#%u  %u votes",
            station.voted_rank + 1, station.votes);
    else text[0] = '\0';
    ...
    SetVisible(document_, id,
        PlaybackActive(status.playback_state) && status.playing_index == index);
}

void RadioApp::RefreshCards() {
    for (unsigned i = 0; i < CardCount; ++i) RefreshCard(i);
    if (card_stations_[selected_slot_] == InvalidStation) selected_slot_ = 0;
}
...
void RadioApp::RefreshDetail() {
    if (card_stations_[selected_slot_] == InvalidStation) {
        ...
        if (radio_service_get_station(card_stations_[selected_slot_], &station)) {
            ...
            SetText(document_, "detail-badge", *station.country_code ? station.country_code : "AAC");
            ...
            char tag[64];
            FirstValue(station.tags, tag, sizeof(tag));
            std::snprintf(line, sizeof(line), "%s  |  %s %u kbps",
                *tag ? tag : "Music", station.codec, station.bitrate);
            SetText(document_, "detail-codec", line);
            std::snprintf(line, sizeof(line), "%u daily plays  |  %u votes",
                station.click_count, station.votes);
...
void RadioApp::RefreshPlayback(const radio_service_status_t& status) {
    ...
    switch (status.playback_state) {
        ...
        case RADIO_PLAYBACK_STOPPING:
            text = "Stopping stream"; warning = true; break;
    ...
    const bool selected_playing = PlaybackActive(status.playback_state) &&
        card_stations_[selected_slot_] == status.playing_index;
    SetClass(document_, "play-icon", "stop", selected_playing);
    SetClass(document_, "play-button", "playing", selected_playing);
    SetText(document_, "play-label", selected_playing ? "Stop station" :
        PlaybackActive(status.playback_state) ? "Switch station" : "Play station");

    if (PlaybackActive(status.playback_state)) {
        radio_station_t station{};
        if (radio_service_get_station(status.playing_index, &station)) {
    ...
    SetClass(document_, "now-art", "playing",
...
void RadioApp::RefreshConnection(const radio_service_status_t& status) {
    ...
    bool warning = false;
    bool error = false;
    if (status.refreshing) { text = "Updating database"; warning = true; }
    else if (status.catalog_state == RADIO_CATALOG_LOADING) {
        text = "Loading database"; warning = true;
    }
    else if (status.catalog_state == RADIO_CATALOG_CACHED) {
        text = "Local cache ready"; warning = true;
    }
    else if (status.catalog_state == RADIO_CATALOG_ERROR) {
        text = status.station_count ? "Offline cache" : "Database unavailable";
        error = true;
    }
...
void RadioApp::UpdateEqualizer(const radio_service_status_t& status) {
    static const unsigned char wave[16] =
        {18, 28, 42, 61, 44, 30, 52, 67, 48, 24, 38, 58, 72, 49, 32, 22};
...
void RadioApp::UpdateSearch() {
    ...
    auto selected_count = [](const Facet* facets, unsigned count,
        const char* selected, unsigned all) {
    ...
    std::snprintf(text, sizeof(text), "COUNTRY  /  %u %s\n<  %s  >",
        selected_count(country_facets_, country_facet_count_, filter_country_,
            country_facet_count_),
        *filter_country_ ? "stations" : "choices",
        *filter_country_ ? filter_country_ : "All countries");
    SetMultilineText(document_, "filter-label-0", text);
    std::snprintf(text, sizeof(text), "GENRE  /  %u %s\n<  %s  >",
        selected_count(genre_facets_, genre_facet_count_, filter_genre_,
            genre_facet_count_),
        *filter_genre_ ? "stations" : "choices",
        *filter_genre_ ? filter_genre_ : "All genres");
    SetMultilineText(document_, "filter-label-1", text);
    std::snprintf(text, sizeof(text), "LANGUAGE  /  %u %s\n<  %s  >",
        selected_count(language_facets_, language_facet_count_, filter_language_,
            language_facet_count_),
        *filter_language_ ? "stations" : "choices",
        *filter_language_ ? filter_language_ : "All languages");
    ...
    const char* bitrate = filter_bitrate_ == 0 ? "Any bitrate" :
        ...
        filter_bitrate_ == 192 ? "192+ kbps" : "256+ kbps";
...
void RadioApp::OpenSearch(unsigned filter) {
    if (search_open_ || credits_open_) return;
    search_open_ = true;
...
void RadioApp::ToggleFavorite() {
    if (card_stations_[selected_slot_] == InvalidStation) return;
...
void RadioApp::CycleFilter(unsigned filter, int direction) {
    auto cycle = [direction](char* selected, std::size_t capacity,
        const Facet* facets, unsigned count) {
        if (!count) { selected[0] = '\0'; return; }
        int current = -1;
        for (unsigned i = 0; i < count; ++i)
            if (std::strcmp(selected, facets[i].value) == 0) current = static_cast<int>(i);
        int next = current + direction;
        if (next < -1) next = static_cast<int>(count) - 1;
        if (next >= static_cast<int>(count)) next = -1;
        if (next < 0) selected[0] = '\0';
        else CopyString(selected, capacity, facets[next].value);
    };
    if (filter == 0)
        cycle(filter_country_, sizeof(filter_country_), country_facets_, country_facet_count_);
    else if (filter == 1)
        cycle(filter_genre_, sizeof(filter_genre_), genre_facets_, genre_facet_count_);
    else if (filter == 2)
        ...
        for (unsigned i = 0; i < sizeof(rates) / sizeof(rates[0]); ++i)
            if (rates[i] == filter_bitrate_) current = static_cast<int>(i);
        current += direction;
        if (current < 0) current = static_cast<int>(sizeof(rates) / sizeof(rates[0])) - 1;
        if (current >= static_cast<int>(sizeof(rates) / sizeof(rates[0]))) current = 0;
...
void RadioApp::HandleSearchKey(radio_input_key_t key) {
    if (key == RADIO_INPUT_CIRCLE) { CloseSearch(false); return; }
    ...
    if (key == RADIO_INPUT_UP) search_focus_ = search_focus_ ? search_focus_ - 1 : 6;
        ...
        if (search_focus_ == 0)
            ...
            search_edit_[0] = filter_country_[0] = filter_genre_[0] =
                filter_language_[0] = '\0';
            filter_bitrate_ = 0;
            UpdateSearch();
        }
...
void RadioApp::HandleMainKey(radio_input_key_t key) {
    if (key == RADIO_INPUT_TRIANGLE) { OpenSearch(InvalidStation); return; }
    if (key == RADIO_INPUT_OPTIONS) { radio_service_refresh(); return; }
    if (key == RADIO_INPUT_L1) { SetView(-1); return; }
    if (key == RADIO_INPUT_R1) { SetView(1); return; }
    if (key == RADIO_INPUT_SQUARE) { ToggleFavorite(); return; }
    ...
    if (focus_ < CardCount) {
        ...
        if (key == RADIO_INPUT_CROSS) TogglePlayback();
        else if (key == RADIO_INPUT_LEFT && (slot & 1U)) focus_ = selected_slot_ = slot - 1;
        else if (key == RADIO_INPUT_RIGHT) {
            if (!(slot & 1U) && card_stations_[slot + 1] != InvalidStation)
                focus_ = selected_slot_ = slot + 1;
            else focus_ = kFocusPlay;
        }
        else if (key == RADIO_INPUT_UP) {
            if (slot >= 2) focus_ = selected_slot_ = slot - 2;
            else { ChangePage(-1, slot + 2); return; }
        }
            ...
            else if (view_ == View::Discover) focus_ = kFocusDiscover + slot % 3;
            else focus_ = kFocusPlay;
        }
        RefreshDetail();
    }
        ...
        if (key == RADIO_INPUT_CROSS) OpenSearch(filter);
        else if (key == RADIO_INPUT_LEFT && filter) --focus_;
        else if (key == RADIO_INPUT_RIGHT && filter < 2) ++focus_;
        else if (key == RADIO_INPUT_UP) focus_ = selected_slot_;
        else if (key == RADIO_INPUT_DOWN) focus_ = kFocusPlay;
    }
...
void RadioApp::Poll() {
    ...
    if (!have_last_status_ ||
        ...
        status.channels != last_status_.channels ||
