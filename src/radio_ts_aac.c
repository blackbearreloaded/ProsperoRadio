...
typedef radio_ts_aac_result_t (*psi_ready_fn)(radio_ts_aac_parser_t * parser,
...
static radio_ts_aac_result_t pat_ready(radio_ts_aac_parser_t * parser,
                                       const uint8_t * section, size_t size)
{
    if(size < 12U || section[0] != 0x00U ||
       (section[1] & 0x80U) == 0U || mpeg_crc32(section, size) != 0U)
        return RADIO_TS_AAC_MALFORMED;
    if((section[5] & 0x01U) == 0U) return RADIO_TS_AAC_OK;

    const size_t entries_end = size - 4U;
    for(size_t at = 8U; at + 4U <= entries_end; at += 4U) {
        const uint16_t program = (uint16_t)((section[at] << 8U) |
                                            section[at + 1U]);
        if(program == 0U) continue;
        const uint16_t pid = (uint16_t)(((section[at + 2U] & 0x1fU) << 8U) |
                                        section[at + 3U]);
...
static radio_ts_aac_result_t pmt_ready(radio_ts_aac_parser_t * parser,
                                       const uint8_t * section, size_t size)
{
    if(size < 16U || section[0] != 0x02U ||
       (section[1] & 0x80U) == 0U || mpeg_crc32(section, size) != 0U)
        return RADIO_TS_AAC_MALFORMED;
    if((section[5] & 0x01U) == 0U) return RADIO_TS_AAC_OK;

    const size_t entries_end = size - 4U;
    const size_t program_info = (size_t)((section[10] & 0x0fU) << 8U) |
                                section[11];
    size_t at = 12U + program_info;
    if(at > entries_end) return RADIO_TS_AAC_MALFORMED;
    while(at + 5U <= entries_end) {
        const uint8_t stream_type = section[at];
        const uint16_t pid = (uint16_t)(((section[at + 1U] & 0x1fU) << 8U) |
                                        section[at + 2U]);
        const size_t info = (size_t)((section[at + 3U] & 0x0fU) << 8U) |
                            section[at + 4U];
...
static radio_ts_aac_result_t psi_append(radio_ts_aac_parser_t * parser,
                                        radio_ts_psi_t * psi,
                                        const uint8_t * data, size_t size,
                                        psi_ready_fn ready, size_t * consumed)
{
    *consumed = 0U;
    while(*consumed < size) {
        ...
        if(wanted > sizeof(psi->data)) return RADIO_TS_AAC_MALFORMED;
        ...
        psi->size += copy;
        *consumed += copy;
        if(psi->size < wanted) return RADIO_TS_AAC_OK;

        if(psi->expected == 0U) {
            const size_t section_length =
                (size_t)((psi->data[1] & 0x0fU) << 8U) | psi->data[2];
        ...
        const radio_ts_aac_result_t result =
            ready(parser, psi->data, psi->expected);
        psi->size = 0U;
        psi->expected = 0U;
        if(result != RADIO_TS_AAC_OK) return result;
    }
    return RADIO_TS_AAC_OK;
}

static radio_ts_aac_result_t psi_feed(radio_ts_aac_parser_t * parser,
                                      radio_ts_psi_t * psi,
                                      const uint8_t * data, size_t size,
                                      bool start, psi_ready_fn ready)
{
    if(start) {
        if(size == 0U) return RADIO_TS_AAC_MALFORMED;
        const size_t pointer = data[0];
        ++data;
        --size;
        if(pointer > size) return RADIO_TS_AAC_MALFORMED;
        if(psi->size != 0U && pointer != 0U) {
            size_t consumed = 0U;
            const radio_ts_aac_result_t result = psi_append(
                parser, psi, data, pointer, ready, &consumed);
            if(result != RADIO_TS_AAC_OK) return result;
    ...
    else if(psi->size == 0U) return RADIO_TS_AAC_OK;

    while(size != 0U) {
        size_t consumed = 0U;
        const radio_ts_aac_result_t result =
            psi_append(parser, psi, data, size, ready, &consumed);
        if(result != RADIO_TS_AAC_OK) return result;
        if(consumed == 0U) return RADIO_TS_AAC_MALFORMED;
        data += consumed;
        size -= consumed;
    }
    return RADIO_TS_AAC_OK;
...
static radio_ts_aac_result_t packet_ready(radio_ts_aac_parser_t * parser,
                                           const uint8_t * packet)
{
    ...
    if(adaptation == 0U) return RADIO_TS_AAC_MALFORMED;
    ...
    if(pid != parser->aac_pid) return RADIO_TS_AAC_OK;

    if(start) {
        parser->pes_started = 0U;
        parser->pes_remaining = 0U;
        if(size < 9U || payload[0] != 0x00U || payload[1] != 0x00U ||
           payload[2] != 0x01U || (payload[3] & 0xe0U) != 0xc0U)
            return RADIO_TS_AAC_MALFORMED;
        const size_t header = 9U + payload[8];
        if(header > size) return RADIO_TS_AAC_UNSUPPORTED;
        const uint32_t packet_length = (uint32_t)(payload[4] << 8U) |
                                       payload[5];
        const uint32_t header_after_length = 3U + payload[8];
        if(packet_length != 0U && packet_length < header_after_length)
            return RADIO_TS_AAC_MALFORMED;
        parser->pes_remaining = packet_length == 0U
            ? UINT32_MAX : packet_length - header_after_length;
        payload += header;
        size -= header;
        parser->pes_started = 1U;
    }
    if(parser->pes_started == 0U || size == 0U) return RADIO_TS_AAC_OK;
    if(size > parser->pes_remaining) size = parser->pes_remaining;
    if(size == 0U) {
        parser->pes_started = 0U;
        return RADIO_TS_AAC_OK;
    }
    ...
    if(parser->pes_remaining != UINT32_MAX) {
        ...
        if(parser->pes_remaining == 0U) parser->pes_started = 0U;
    }
    return RADIO_TS_AAC_OK;
}
...
radio_ts_aac_result_t radio_ts_aac_feed(radio_ts_aac_parser_t * parser,
                                        const uint8_t * data, size_t size)
{
    if(parser == NULL || parser->output == NULL ||
       (data == NULL && size != 0U)) return RADIO_TS_AAC_INVALID;
    while(size != 0U) {
        ...
        data += copy;
        size -= copy;
        if(parser->packet_size != RADIO_TS_PACKET_BYTES) continue;

        if(parser->packet[0] != 0x47U) {
            size_t sync = 1U;
            while(sync < RADIO_TS_PACKET_BYTES &&
                  parser->packet[sync] != 0x47U) ++sync;
            memmove(parser->packet, parser->packet + sync,
                    RADIO_TS_PACKET_BYTES - sync);
