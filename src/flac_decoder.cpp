...
{
    auto *decoder = static_cast<flac_decoder_t *>(user_data);
    ...
    auto *header =
        static_cast<flac_allocation_header_t *>(malloc(sizeof(flac_allocation_header_t) + size));
    if (header == nullptr)
        return nullptr;
...
static void flac_free(void *memory, void *user_data)
{
    if (memory == nullptr)
        return;
    auto *decoder = static_cast<flac_decoder_t *>(user_data);
    auto *header = static_cast<flac_allocation_header_t *>(memory) - 1;
...
{
    if (memory == nullptr)
        return flac_malloc(new_size, user_data);
    if (new_size == 0U)
    {
    ...
    const size_t without_old =
        decoder->allocation_bytes >= old_size ? decoder->allocation_bytes - old_size : 0U;
    ...
    header = static_cast<flac_allocation_header_t *>(realloc(header, sizeof(*header) + new_size));
    if (header == nullptr)
        return nullptr;
...
static size_t flac_read_callback(void *user_data, void *output, size_t bytes_to_read)
{
    auto *decoder = static_cast<flac_decoder_t *>(user_data);
    ...
    {
        if (decoder->opening && decoder->stream_position >= FLAC_DECODER_OPEN_READ_LIMIT)
        {
            decoder->read_error = FLAC_DECODER_OPEN_READ_LIMIT_EXCEEDED;
            break;
        }
        size_t request = bytes_to_read - total;
        if (request > FLAC_DECODER_IO_CHUNK)
            request = FLAC_DECODER_IO_CHUNK;
        if (decoder->opening)
        {
            const size_t opening_remaining =
                FLAC_DECODER_OPEN_READ_LIMIT - (size_t)decoder->stream_position;
            if (request > opening_remaining)
                request = opening_remaining;
        }
        const int received = decoder->read(decoder->read_context, bytes + total, request);
        if (received < 0)
        {
...
static drflac_bool32 flac_seek_callback(void *user_data, int offset, drflac_seek_origin origin)
{
    auto *decoder = static_cast<flac_decoder_t *>(user_data);
    ...
    if (target < decoder->stream_position)
        return DRFLAC_FALSE;
    ...
    while (decoder->stream_position < target)
    {
        ...
        const size_t request = remaining < sizeof(discard) ? (size_t)remaining : sizeof(discard);
...
int flac_decoder_open(flac_decoder_t *decoder, flac_decoder_read_fn read, void *read_context)
{
    if (decoder == nullptr || read == nullptr || decoder->handle != nullptr)
        return FLAC_DECODER_ERROR;
    memset(decoder, 0, sizeof(*decoder));
    decoder->read = read;
    decoder->read_context = read_context;
    decoder->opening = true;
    const drflac_allocation_callbacks allocation = {decoder, flac_malloc, flac_realloc, flac_free};
    drflac *handle =
        drflac_open(flac_read_callback, flac_seek_callback, nullptr, decoder, &allocation);
    decoder->opening = false;
    if (handle != nullptr && decoder->read_error < 0)
    {
    ...
    {
        const int result =
            decoder->read_error != 0 ? decoder->read_error : FLAC_DECODER_OPEN_FAILED;
        memset(decoder, 0, sizeof(*decoder));
        return result;
    }

    if (handle->sampleRate < 8000U || handle->sampleRate > 192000U || handle->channels < 1U ||
        handle->channels > 2U || handle->bitsPerSample < 4U || handle->bitsPerSample > 32U ||
...
int flac_decoder_read_pcm(flac_decoder_t *decoder, int16_t *pcm, size_t pcm_capacity_samples,
                          size_t *pcm_samples)
{
    if (decoder == nullptr || decoder->handle == nullptr || pcm == nullptr ||
        pcm_samples == nullptr || decoder->channels == 0U)
        return FLAC_DECODER_ERROR;
    *pcm_samples = 0U;
    size_t frames = pcm_capacity_samples / decoder->channels;
    if (frames == 0U)
        return FLAC_DECODER_OUTPUT_TOO_SMALL;
    if (frames > FLAC_DECODER_READ_FRAMES)
        frames = FLAC_DECODER_READ_FRAMES;
    const drflac_uint64 produced = drflac_read_pcm_frames_s16(
        static_cast<drflac *>(decoder->handle), (drflac_uint64)frames, pcm);
    if (produced == 0U)
    {
        if (decoder->read_error < 0)
            return decoder->read_error;
...
void flac_decoder_close(flac_decoder_t *decoder)
{
    ...
    if (decoder->handle != nullptr)
        drflac_close(static_cast<drflac *>(decoder->handle));
    memset(decoder, 0, sizeof(*decoder));
}
