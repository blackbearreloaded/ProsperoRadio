...
static ogg_page_result_t fail(ogg_page_parser_t *parser, ogg_page_result_t error)
{
    parser->error = error;
    return error;
}
...
static uint32_t page_crc(const uint8_t *data, size_t size)
{
    ...
    for (size_t i = 0; i < size; ++i)
    {
        ...
        {
            crc = (crc << 1) ^ ((crc & UINT32_C(0x80000000)) != 0U ? OGG_CRC_POLYNOMIAL : 0U);
        }
    }
    return crc;
}
...
static ogg_page_result_t advance(ogg_page_parser_t *parser)
{
    ...
    if (page_crc(parser->data, parser->target) != read_u32le(parser->data + 22))
    ...
    if (parser->on_page != nullptr && parser->on_page(&page, parser->user_data) != 0)
...
void ogg_page_init(ogg_page_parser_t *parser, ogg_page_fn on_page, void *user_data)
{
    if (parser == nullptr)
        return;
...
ogg_page_result_t ogg_page_feed(ogg_page_parser_t *parser, const void *input, size_t size)
{
