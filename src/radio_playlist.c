...
static bool contains_case(const char * data, size_t size, const char * text)
{
    ...
    for(size_t at = 0; at + length <= size; ++at) {
        ...
        while(matched < length &&
              ascii_lower(data[at + matched]) == ascii_lower(text[matched])) {
            ++matched;
        }
...
static bool valid_url_text(const char * value, size_t size)
{
    ...
    for(size_t i = 0; i < size; ++i) {
        ...
        if(byte <= 0x20U || byte == 0x7fU || value[i] == '#' ||
...
{
    size_t end = 0;
    while(url[end] != '\0' && url[end] != '?' && url[end] != '#') ++end;
    const size_t length = strlen(extension);
    return end >= length && equal_case(
        (text_slice_t){url + end - length, length}, extension);
}

radio_playlist_kind_t radio_playlist_kind_from_url(const char * url)
{
    if(url == NULL) return RADIO_PLAYLIST_NONE;
    if(extension_is(url, ".m3u8")) return RADIO_PLAYLIST_HLS;
    if(extension_is(url, ".m3u")) return RADIO_PLAYLIST_M3U;
    if(extension_is(url, ".pls")) return RADIO_PLAYLIST_PLS;
...
radio_playlist_kind_t radio_playlist_kind_from_headers(const char * headers,
                                                        size_t size)
{
    if(headers == NULL || size == 0) return RADIO_PLAYLIST_NONE;
...
radio_playlist_kind_t radio_playlist_kind_from_body(const char * data,
                                                     size_t size)
{
    if(data == NULL || size == 0) return RADIO_PLAYLIST_NONE;
    if(size >= 3U && memcmp(data, "\xef\xbb\xbf", 3U) == 0) {
        data += 3U;
        size -= 3U;
    }
    if(contains_case(data, size, "#EXT-X-")) return RADIO_PLAYLIST_HLS;
    if(contains_case(data, size, "[playlist]") &&
       contains_case(data, size, "file1=")) return RADIO_PLAYLIST_PLS;
    if(contains_case(data, size, "#EXTM3U")) return RADIO_PLAYLIST_M3U;
    return RADIO_PLAYLIST_NONE;
}

static radio_playlist_result_t copy_url(text_slice_t value,
                                        char * output, size_t output_size)
{
    if(!valid_url_text(value.data, value.size)) return RADIO_PLAYLIST_INVALID;
    if(value.size + 1U > output_size) return RADIO_PLAYLIST_LIMIT;
...
radio_playlist_result_t radio_playlist_resolve_url(
    ...
    char * output, size_t output_size)
{
    if(base_url == NULL || reference == NULL || output == NULL ||
       output_size == 0) return RADIO_PLAYLIST_INVALID;

    text_slice_t ref = trim((text_slice_t){reference, strlen(reference)});
    if(!valid_url_text(ref.data, ref.size)) return RADIO_PLAYLIST_INVALID;
    if(http_url(ref)) return copy_url(ref, output, output_size);
    for(size_t i = 0; i < ref.size; ++i) {
        if(ref.data[i] == ':') return RADIO_PLAYLIST_INVALID;
        if(ref.data[i] == '/' || ref.data[i] == '?') break;
    }

    const char * scheme = strstr(base_url, "://");
    if(scheme == NULL || (!starts_case((text_slice_t){base_url, strlen(base_url)},
                                       "http://") &&
                          !starts_case((text_slice_t){base_url, strlen(base_url)},
                                       "https://")))
    ...
    while(path < base_end && *path != '/' && *path != '?') ++path;
    if(path == authority) return RADIO_PLAYLIST_INVALID;
    ...
    if(ref.size >= 2U && ref.data[0] == '/' && ref.data[1] == '/') {
        ...
        const char * end = path;
        while(end < base_end && *end != '?') ++end;
        const char * slash = end;
        while(slash > path && slash[-1] != '/') --slash;
        prefix = slash > path ? (size_t)(slash - base_url)
                              : (size_t)(path - base_url);
    ...
    if(insert_slash) output[prefix++] = '/';
    memcpy(output + prefix, ref.data, ref.size);
    output[prefix + ref.size] = '\0';
    return http_url((text_slice_t){output, prefix + ref.size})
        ? RADIO_PLAYLIST_OK : RADIO_PLAYLIST_INVALID;
}

static bool pls_entry(text_slice_t line, text_slice_t * value)
{
    size_t equals = 0;
    while(equals < line.size && line.data[equals] != '=') ++equals;
    if(equals == line.size) return false;
    text_slice_t key = trim((text_slice_t){line.data, equals});
    if(key.size <= 4U || !starts_case(key, "file")) return false;
    for(size_t i = 4U; i < key.size; ++i) {
        if(key.data[i] < '0' || key.data[i] > '9') return false;
    }
    *value = trim((text_slice_t){line.data + equals + 1U,
                                 line.size - equals - 1U});
    return value->size != 0;
}
    ...
    const char * playlist_url, char * output, size_t output_size)
{
    if((kind != RADIO_PLAYLIST_M3U && kind != RADIO_PLAYLIST_PLS) ||
       data == NULL || size == 0 || playlist_url == NULL || output == NULL)
        return RADIO_PLAYLIST_INVALID;
    if(radio_playlist_kind_from_body(data, size) == RADIO_PLAYLIST_HLS)
        return RADIO_PLAYLIST_IS_HLS;
    if(memchr(data, '\0', size) != NULL) return RADIO_PLAYLIST_INVALID;
    ...
    while(position < size) {
        ...
        text_slice_t entry = {NULL, 0};
        if(kind == RADIO_PLAYLIST_M3U) {
            if(line.size != 0 && line.data[0] != '#') entry = line;
        }
        ...
        if(entry.size != 0) {
            char reference[1024];
            if(entry.size + 1U > sizeof(reference)) return RADIO_PLAYLIST_LIMIT;
            memcpy(reference, entry.data, entry.size);
            reference[entry.size] = '\0';
            return radio_playlist_resolve_url(
                playlist_url, reference, output, output_size);
        }
